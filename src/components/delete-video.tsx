...
export function DeleteVideo(){
    const [categories, setCategories] = useState<CategoryContract[]>();
        ...
        const formik = useFormik({
            ...
            onSubmit : (video) => {
                axios.put(`http://127.0.0.1:5070/edit-video/${params.id}`, video)
                .then(()=>{
        ...
        function LoadVideos(){
            axios.get(`http://127.0.0.1:5070/get-video/${params.id}`)
            .then(response =>{ 
        ...
        function handleDeleteClick(){
            axios.delete(`http://127.0.0.1:5070/delete-video/${params.id}`)
            .then(()=>{
                console.log('deleted');
            })
            alert('Video Deleted' + videos[0].title + ' Successfully.');
            navigate('/admin-dashboard');
        }
